...
Outputs:
  ...
  3. tai_diagram.png            — TAI maturity diagram with Palynomorph thumbnails
...
FIELD_BOUNDARIES = [
    [(0,20,80),  (20,20,60), "solid"],
    [(20,0,80),  (20,20,60), "solid"],
    [(20,20,60), (20,40,40), "solid"],
    [(20,40,40), (40,40,20), "solid"],
    [(0,40,60),  (20,40,40), "solid"],
    [(0,60,40),  (20,60,20), "solid"],
    [(20,60,20), (40,60,0),  "solid"],
    [(20,0,80),  (60,0,40),  "solid"],
    [(35,20,45), (40,20,40), "solid"],
    [(40,20,40), (40,40,20), "solid"],
    [(40,40,20), (80,20,0),  "solid"],
    [(60,0,40),  (80,0,20),  "solid"],
    [(20,20,60), (35,20,45), "dashed"],
    [(35,20,45), (55,15,30), "dashed"],
]
...
# IMAGE ANALYSIS
# ══════════════════════════════════════════════════════════════════════════════
def analyze_image(model, img_path: Path, mag: int) -> tuple:
    ...
    for r in results:
        ...
        for mask_t, cls_id in zip(r.masks.data, r.boxes.cls):
            ...
            if cls_name == "Palynomorph" and img_rgb is not None:
                if mask_np.shape != img_rgb.shape[:2]:
                    mask_np_full = cv2.resize(mask_np.astype(np.uint8),
                                         (img_rgb.shape[1], img_rgb.shape[0]),
                                         interpolation=cv2.INTER_NEAREST)
                else:
                    mask_np_full = mask_np.astype(np.uint8)

                tai, status = estimate_tai(img_rgb, mask_np)
                if tai is not None:
                    tai_scores.append(tai)
                
                # Extract original thumbnail crop of the largest palynomorph
                area = float(mask_np_full.sum())
                if area > largest_pal_area:
                    largest_pal_area = area
                    y_idx, x_idx = np.where(mask_np_full == 1)
                    if len(y_idx) > 0 and len(x_idx) > 0:
                        ymin, ymax = y_idx.min(), y_idx.max()
                        xmin, xmax = x_idx.min(), x_idx.max()
                        best_thumb = img_rgb[ymin:ymax+1, xmin:xmax+1]
...
# ══════════════════════════════════════════════════════════════════════════════
# CHART 3: TAI DIAGRAM WITH THUMBNAILS
# ══════════════════════════════════════════════════════════════════════════════
def plot_tai_diagram(df: pd.DataFrame, out_path: Path, palyno_thumbs: dict = None):
    valid = df[df["Estimated_TAI"].notna()].copy()
    n = len(valid)
    
    # Dynamic height adjustment based on sample counts to keep thumbnails readable
    fig_height = max(7, n * 0.45 + 2) if n > 0 else 7
    ...
    if len(valid) > 0:
        ...
        ax1.set_title("Sample TAI Values & Palynomorphs", fontsize=10,
                      fontweight="bold", fontfamily="serif")
        ax1.axvline(tai_vals.mean(), color="red", lw=1.5, ls="--",
                    label=f"Mean TAI = {tai_vals.mean():.2f}")
        ax1.legend(fontsize=8)
        
        # Overlay original palynomorph image crops as thumbnails inside the bar area
        if palyno_thumbs:
            for idx, depth in enumerate(depths):
                img_name = valid.iloc[idx]["image"]
                thumb = palyno_thumbs.get(img_name)
                if thumb is not None:
                    try:
                        # Resize cleanly to standard 35x35 px thumbnail size
                        thumb_resized = cv2.resize(thumb, (35, 35), interpolation=cv2.INTER_AREA)
                        imagebox = OffsetImage(thumb_resized, zoom=1.0)
                        # Center thumbnail at x=1.15 to stay beautifully visible inside the color bar
                        ab = AnnotationBbox(imagebox, (1.15, depth), frameon=True, pad=1,
                                            bboxprops=dict(edgecolor="gray", lw=0.5, facecolor="white"))
                        ax1.add_artist(ab)
                    except Exception:
                        pass
...
def main():
    ...
    for i, img_path in enumerate(images, 1):
        ...
        row, thumb = analyze_image(model, img_path, mag)
        rows.append(row)
        if thumb is not None:
            palyno_thumbs[img_path.name] = thumb
    ...
    plot_tai_diagram(df, out_dir / "tai_diagram.png", palyno_thumbs)
    ...
    print("   ├── tai_diagram.png           (TAI maturity with thumbnails)")
