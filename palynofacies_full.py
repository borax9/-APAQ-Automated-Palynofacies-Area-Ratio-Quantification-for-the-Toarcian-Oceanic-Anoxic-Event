...
====================
YOLO26 segmentation modeliyle tam Palynofacies + TAI analizi.

Çıktılar:
  1. tyson_ternary_plot.png     — Tyson (1995) 9 alanlı üçgen (görseldeki gibi)
  2. relative_abundance_log.png — Jeolojik log formatında relative abundance
  3. tai_diagram.png            — TAI olgunluk diyagramı
...
COLORS = {
    "AOM"        : "#8B0000",    # koyu kırmızı — jeolojik standart
    "Palynomorph": "#1A5E20",    # koyu yeşil
    "Phytoclast" : "#1A3A6B",    # koyu mavi
    "Background" : "#AAAAAA",
}

# Kalibrasyon: mikrometre cetveli ile ölçülmüş
CALIBRATION = {
...
# ══════════════════════════════════════════════════════════════════════════════
# TYSON (1995) ÜÇGEN — GÖRSELE BIREBIR UYGUN KOORDİNATLAR
# Köşeler: AOM=sol alt, Palynomorphs=sağ alt, Phytoclasts=üst
# Referans: Tyson R.V. (1995) Sedimentary Organic Matter, Chapman & Hall
# ══════════════════════════════════════════════════════════════════════════════
# Görseldeki alan sınırları (Tyson 1995 Fig. 11.1):
#   Phytoclast ekseni: sol kenar (AOM-Phytoclast)
#   %20 Phytoclast çizgisi: alt yatay çizgi (IX+VIII+VII+V sınırı)
#   %40 Phytoclast çizgisi: ikinci yatay çizgi
#   AOM %20 dikey: I/II sınırı üst kısım
#   Palynomorph %60: V/III+VIII sınırı
#   Alan IV: merkez geçiş bölgesi (IVa ve IVb)

TYSON_FIELDS = [
    # ── Üst bölge: Phytoclast dominant ──────────────────────────────────────
    {   # I: Phytoclast köşesine yakın, AOM<20, Pal<20
        "num": "I", "color": "#FFFFFF",
        "name": "Highly proximal\nshelf or basin",
        "pts": [(0,0,100),(20,0,80),(15,15,70),(0,15,85)],
        "label_pos": (8, 5, 87),
    },
    {   # II: Sol orta üst — Phytoclast>40, AOM<20
        "num": "II", "color": "#FFFFFF",
        "name": "Marginal dysoxic-\noxic shelf",
        "pts": [(0,15,85),(15,15,70),(20,20,60),(20,40,40),(0,40,60),(0,20,80)],
        "label_pos": (8, 25, 67),
    },
    {   # III: Sağ orta — Pal>40, Phy>20
        "num": "III", "color": "#FFFFFF",
        "name": "Heterolithic oxic\nshelf (proximal)",
        "pts": [(0,40,60),(20,40,40),(20,60,20),(0,60,40)],
        "label_pos": (8, 52, 40),
    },
    {   # IVa: Merkez üst geçiş
        "num": "IVa", "color": "#FFFFFF",
        "name": "Shelf to basin\ntransition",
        "pts": [(15,15,70),(20,0,80),(40,0,60),(35,20,45),(20,20,60)],
        "label_pos": (26, 12, 62),
    },
    {   # IVb: Merkez alt geçiş
        "num": "IVb", "color": "#FFFFFF",
        "name": "Shelf to basin\ntransition",
        "pts": [(20,20,60),(35,20,45),(40,20,40),(40,40,20),(20,40,40)],
        "label_pos": (30, 30, 40),
    },
    {   # V: Sağ alt — Pal dominant >60
        "num": "V", "color": "#FFFFFF",
        "name": "Mud-dominated oxic\nshelf (distal)",
        "pts": [(0,60,40),(20,60,20),(20,80,0),(0,80,20),(0,100,0)],
        "label_pos": (8, 72, 20),
    },
    {   # VI: Sol orta — AOM 20-60, Phy 20-60
        "num": "VI", "color": "#FFFFFF",
        "name": "Proximal suboxic-\nanoxic shelf",
        "pts": [(20,0,80),(60,0,40),(55,15,30),(35,20,45),(20,20,60),(15,15,70)],
        "label_pos": (35, 8, 57),
    },
    {   # VII: Merkez alt — AOM 40-80, Pal 20-40
        "num": "VII", "color": "#FFFFFF",
        "name": "Distal dysoxic-\nanoxic shelf",
        "pts": [(40,20,40),(55,15,30),(60,0,40),(80,0,20),(60,20,20),(40,40,20),(40,20,40)],
        "label_pos": (58, 18, 24),
    },
    {   # VIII: Alt orta — AOM 20-60, Pal 40-80
        "num": "VIII", "color": "#FFFFFF",
        "name": "Distal dysoxic-\nanoxic basin",
        "pts": [(20,40,40),(40,40,20),(40,60,0),(20,80,0),(20,60,20)],
        "label_pos": (30, 55, 15),
    },
    {   # IX: Sol alt köşe — AOM dominant >60
        "num": "IX", "color": "#FFFFFF",
        "name": "Distal suboxic-\nanoxic basin",
...
# Alan sınır çizgileri (görseldeki kesik çizgi dahil)
FIELD_BOUNDARIES = [
    # (başlangıç AOM,Pal,Phy) → (bitiş AOM,Pal,Phy), stil
    [(0,20,80),  (20,20,60), "solid"],   # I/II alt sınır
    [(20,0,80),  (20,20,60), "solid"],   # I/IVa sağ sınır
    [(20,20,60), (20,40,40), "solid"],   # II/IVb sol sınır
    [(20,40,40), (40,40,20), "solid"],   # III-IVb/VIII sınır
    [(0,40,60),  (20,40,40), "solid"],   # II/III sınır
    [(0,60,40),  (20,60,20), "solid"],   # III/V sınır
    [(20,60,20), (40,60,0),  "solid"],   # V/VIII sınır
    [(20,0,80),  (60,0,40),  "solid"],   # IVa-VI orta sınır
    [(35,20,45), (40,20,40), "solid"],   # IVa/IVb-VII geçiş
    [(40,20,40), (40,40,20), "solid"],   # VII/IVb+VIII sınır
    [(40,40,20), (80,20,0),  "solid"],   # VII/VIII+IX sınır
    [(60,0,40),  (80,0,20),  "solid"],   # VI/VII+IX sınır
    [(20,20,60), (35,20,45), "dashed"],  # IVa/IVb iç sınır (kesik)
    [(35,20,45), (55,15,30), "dashed"],  # iç geçiş kesik çizgi
]
...
# ══════════════════════════════════════════════════════════════════════════════
def tc(aom, pal, phy):
    """Ternary (AOM, Pal, Phy) → Kartezyen (x, y).
    AOM=sol alt, Palynomorphs=sağ alt, Phytoclasts=üst köşe.
    """
    total = aom + pal + phy
    if total == 0:
        return 0.5, 0.0
...
def estimate_tai(image_rgb, mask_np):
    """Palynomorph maskesi altındaki piksellerin renk analiziyle TAI tahmini."""
...
# ══════════════════════════════════════════════════════════════════════════════
# GRAFİK 1: TYSON ÜÇGEN — GÖRSELE UYGUN
# ══════════════════════════════════════════════════════════════════════════════
def draw_tyson_ternary(ax):
    """Tyson (1995) üçgenini görseldeki gibi çiz — beyaz arka plan, siyah çizgiler."""

    # Tüm alanları beyaz dolgu + ince sınır ile çiz
    ...
    # Alan sınır çizgileri (görseldeki gibi siyah, bazıları kesik)
...
# ══════════════════════════════════════════════════════════════════════════════
# GRAFİK 2: JEOLOJİK LOG — RELATIVE ABUNDANCE
# ══════════════════════════════════════════════════════════════════════════════
def plot_relative_abundance_log(df: pd.DataFrame, out_path: Path):
    """Jeolojik log formatında relative abundance curve grafikleri."""
    n = len(df)
    depths = np.arange(1, n + 1)  # Görüntü indeksi (derinlik yerine)
    ...
    # Grid: 6 kolon (AOM%, Pal%, Phy%, AOM count, Pal count, Phy count) + 1 fasies kolonu
    ...
    for col_i, (col, title, color, xmin, xmax) in enumerate(col_configs):
        ...
        # Sadece ilk kolonda y ekseni etiketleri
        if col_i == 0:
        ...
        ax.text(mean_val + 1, 0.3, f"μ={mean_val:.1f}",
                fontsize=6, color=color, va="top")

    # Fasies kolonu (en sağ)
...
# ══════════════════════════════════════════════════════════════════════════════
# GRAFİK 3: TAI DİYAGRAMI
# ══════════════════════════════════════════════════════════════════════════════
def plot_tai_diagram(df: pd.DataFrame, out_path: Path):
    """TAI olgunluk diyagramı — jeolojik standart."""
    ...
    # ── Orta: Örnek TAI dağılımı log formatında ───────────────────────────
    ax1 = axes[1]
    if len(valid) > 0:
        n = len(valid)
        ...
        # TAI skalası arka plan renklendirmesi
    ...
    # ── Sağ: TAI dağılım histogramı + özet ───────────────────────────────
    ax2 = axes[2]
    if len(valid) > 0:
        tai_vals = valid["Estimated_TAI"].values
...
def print_summary(df: pd.DataFrame, mag: int):
    ...
    print("\n" + "="*70)
    print(f"PALYNOFACIES & TAI ANALİZ SONUÇLARI — Tyson (1995)")
    print(f"Büyütme: {mag}x  |  Kalibrasyon: {cal['um_per_pixel']} µm/px")
    print("="*70)
    print(f"Toplam görüntü: {len(df)}\n")
    print(f"{'Sınıf':15s} {'Alan%':>8s} {'±std':>6s} "
          f"{'µm²(ort)':>12s} {'Count(ort)':>11s} {'Count%':>8s}")
    print("-"*70)
    for g in PALYNO_GROUPS:
        print(f"{g:15s} "
...
def main():
    ...
    parser.add_argument("--mag",     type=int, default=None,
                        help="Büyütme (10/20/40). Klasör adından algılanabilir.")
    ...
    print(f"  Görüntüler: {len(images)}  |  {mag}x")
    ...
    print("   ├── tyson_ternary_plot.png    (Tyson 1995, 9 alan)")
    print("   ├── relative_abundance_log.png (Jeolojik log)")
    print("   ├── tai_diagram.png           (TAI olgunluk)")
    print("   ├── summary_charts.png        (İstatistik)")
    print("   └── palynofacies_report.xlsx  (6 sheet)")
